  ...
  function initPost() {   // AAA pipeline: scene -> RT, bright-pass -> quarter-res separable gaussian BLOOM, then film composite (CA, grain, vignette, tilt-shift, halation, split-tone grade)
    try {
      ...
      var fsh = [
        ...
        '  float edge = 0.0;',
        '  if (uPost > 0.5 || uEdge > 0.01) {',                                                                    // STYLISED modes only: blur the texture grain away, then posterize/edge the SMOOTH image (no grain speckle)
        '    vec2 e2 = 3.0 / uRes;',
        ...
        '    vec3 bc = (t00+t10+t20+t01+col+t21+t02+t12+t22) * 0.1111;',
        '    if (uPost > 0.5) col = floor(bc * uPost + 0.5) / uPost;',                                             // posterize the BLURRED colour → clean flat bands
        '    if (uEdge > 0.01) { vec3 LW=vec3(0.299,0.587,0.114); float l00=dot(t00,LW),l10=dot(t10,LW),l20=dot(t20,LW),l01=dot(t01,LW),l21=dot(t21,LW),l02=dot(t02,LW),l12=dot(t12,LW),l22=dot(t22,LW); float gx=(l20+2.0*l21+l22)-(l00+2.0*l01+l02); float gy=(l02+2.0*l12+l22)-(l00+2.0*l10+l20); edge=smoothstep(0.5, 1.25, sqrt(gx*gx+gy*gy)*uEdge); }',   // 3x3 Sobel on the SMOOTH grid → only real structural edges, not grain
        '  }',
