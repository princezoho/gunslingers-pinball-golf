...
(function () {
  ...
  function toonSync() {
    ...
    if (on) {
      if (!R3.toonGrad) { R3.toonGrad = toonGrad(4); R3.toonGradC = toonGrad(2, 0.5, 0.82); }   // cel = 4 smooth bands (keeps texture); cartoon = 2 FLAT bands capped at 0.82 so the lit band stays RICH colour (never blows light materials to white) + a real shadow band
      ...
      R3.group.traverse(function (o) {
        ...
        else { var key = (mood === 'cel') ? 'celMat' : 'cartMat'; if (!o.userData[key]) { var fc = src.color ? src.color.clone() : new T.Color(0xffffff); if (mood === 'cartoon') { fc.offsetHSL(0, 0.32, -0.14); var _hsl = {}; fc.getHSL(_hsl); if (_hsl.s < 0.12) fc.offsetHSL(0, 0, -0.12); }   /* punch cartoon flat colours: saturated + darker so they don't blow out; near-grey woods get an extra darken so they read as wood, not white */ o.userData[key] = new T.MeshToonMaterial({ map: (mood === 'cel') ? (src.map || null) : null, color: fc, gradientMap: grad }); } o.material = o.userData[key]; }   // CARTOON drops the texture map → FLAT colour; CEL keeps the map (textured banding)
      });
  ...
  function showDailyCard(rec) {
    ...
    elt('div', 'font:900 26px Wantedo,Georgia;color:#f5c542;margin:2px 0 4px;', S.name, box);   // hole name = the ONE marigold accent; everything else defaults to cream/white
    ...
    if (net && net.enabled && St.dailyDay) {
      ...
      else if (firstTime) elt('div', 'font:800 12px Georgia;color:#f5efdc;margin-bottom:6px;', '🤠 You’re on the board! ✏️ Change your name below + tap POST to claim it.', lbWrap);
    ...
    var xb = elt('button', prim + 'color:#f5efdc;', '𝕏  SHARE ON X', box);
