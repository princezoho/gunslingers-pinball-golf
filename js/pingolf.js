...
(function () {
  ...
  var BUILD = 'BUILD 204 · PENDULUM';

  /* ================================================================ HOLE BUILDER
     A tiny DSL: each hole function fills a builder with obstacles and returns it. */
  function builder() {
    var b = {
      walls: [], bumpers: [], boosters: [], flippers: [], windmills: [], lasers: [], loops: [], warps: [], portals: [], firerings: [], enemies: [], coins: [], powerups: [], spinners: [], cannons: [], bouncers: [], gates: [], conveyors: [], pendulums: [], multiball: null,
  ...
  function edClearAll() {   // wipe all placed obstacles/items but keep the level shape (walls), tee, cup, bounds, theme
    ...
    ['bumpers', 'boosters', 'flippers', 'windmills', 'lasers', 'gates', 'conveyors', 'pendulums', 'loops', 'warps', 'portals', 'firerings', 'enemies', 'coins', 'powerups'].forEach(function (k) { d[k] = []; });
  ...
  function icon(c, kind, s, sel, item) {
    ...
    else if (kind === 'conveyor') { var cl = R(it.len || 480, 10) / 2, cw = R(it.w || 240, 8) / 2, ca = it.ang || 0; c.save(); c.translate(s.x, s.y); c.rotate(ca - PI / 2); c.fillStyle = sel ? 'rgba(245,197,66,.22)' : 'rgba(40,42,48,.5)'; c.strokeStyle = '#f5c542'; c.lineWidth = 2; c.fillRect(-cw, -cl, cw * 2, cl * 2); c.strokeRect(-cw, -cl, cw * 2, cl * 2); c.fillStyle = '#f5c542'; for (var cai = -1; cai <= 1; cai++) { var cy = cai * cl * 0.55; c.beginPath(); c.moveTo(0, cy - 6); c.lineTo(6, cy + 5); c.lineTo(-6, cy + 5); c.closePath(); c.fill(); } c.restore(); }
    else if (kind === 'pendulum') { var sw = R((it.len || 360) * Math.sin(it.amp || 1.1), 6); c.strokeStyle = '#9a9aa2'; c.lineWidth = 3; c.beginPath(); c.moveTo(s.x - sw, s.y); c.lineTo(s.x + sw, s.y); c.stroke(); c.fillStyle = sel ? '#f5c542' : '#33363d'; c.strokeStyle = '#15161a'; c.lineWidth = 1.5; c.beginPath(); c.arc(s.x, s.y, R(it.rb || 46, 4), 0, TAU); c.fill(); c.stroke(); }
  ...
  function showDailyCard(rec) {
    ...
    if (net && net.enabled && St.dailyDay) {
      ...
      function renderTeams() {
        ...
        elt('div', 'font:900 12px Wantedo,Georgia;color:#f5c542;letter-spacing:1px;margin-bottom:6px;text-align:center;', '🏴 TEAMS', teamWrap);
        var team = getTeam();
        if (team && team.id) {
  ...
  function showTeamBallSetup() {
    ...
    var box = elt('div', 'width:390px;max-width:92%;background:#241a0e;border:2px solid #f5c542;border-radius:16px;padding:22px;box-shadow:0 12px 56px rgba(0,0,0,.75);text-align:center;', null, ov);
    ...
    function paintMode() {
      var on = 'flex:1;padding:10px;border:2px solid #f5c542;border-radius:8px;background:linear-gradient(180deg,#8a6a1e,#5a3a10);color:#fff;font:900 13px Wantedo,Georgia;cursor:pointer;';
