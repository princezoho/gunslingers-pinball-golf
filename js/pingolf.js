...
(function () {
  ...
  function initGL(canvas) {
    ...
    try {
      ...
      sun.shadow.bias = -0.0004; if ('normalBias' in sun.shadow) sun.shadow.normalBias = 3;   // a touch more normalBias to kill shadow acne
  ...
  function applyMood(name) {
    ...
    if (R3.post && R3.post.mat) { var u = R3.post.mat.uniforms, g = m.grade; u.uSat.value = g.sat; u.uSep.value = g.sep; u.uShad.value.set(g.shad[0], g.shad[1], g.shad[2]); u.uHigh.value.set(g.high[0], g.high[1], g.high[2]); u.uBloomTint.value.set(1 + (g.high[0] - 1) * 0.7, 1 + (g.high[1] - 1) * 0.7, 1 + (g.high[2] - 1) * 0.7); u.uLo.value = g.lo; u.uHi.value = g.hi; u.uCon.value = g.con * 1.04; u.uBright.value = g.bright; u.uLift.value = g.lift; u.uGrain.value = m.grain; u.uVig.value = m.vig * 0.45; R3.post.ca = m.ca * 0.4;   // gentle contrast + lighter vignette + much less chromatic-aberration fringing (the coloured edge fringe read as "weirdness")
