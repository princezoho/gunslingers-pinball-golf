...
(function () {
  ...
  var BUILD = 'BUILD 29 · LIVE EDITOR · AUTO-UPDATE';
  ...
  function builder() {
    var b = {
      ...
      wall: function (ax, az, bx, bz, o) { o = o || {}; this.walls.push({ ax: ax, az: az, bx: bx, bz: bz, e: o.e == null ? K.wallE : o.e, h: Math.min(o.h || 46, 130), c: o.c || 0x8a5a32 }); return this; },   // default low curbs; editor can raise height
  ...
  function buildScene(hole) {
    ...
    // lasers
    hole.lasers.forEach(function (la) { var dx = la.bx - la.ax, dz = la.bz - la.az, L = hyp(dx, dz), gy = hole.terrain((la.ax + la.bx) / 2, (la.az + la.bz) / 2); var m = new T.Mesh(new T.CylinderGeometry(7, 7, L, 8), new T.MeshStandardMaterial({ color: 0xff2a3a, emissive: 0xff2a3a, emissiveIntensity: .9 })); m.rotation.z = PI / 2; m.rotation.y = Math.atan2(dz, dx); m.position.set((la.ax + la.bx) / 2, gy + 30, (la.az + la.bz) / 2); R3.group.add(m); la.mesh = m; });
    // multiball pad
  ...
  function collideLaser(b, la, gy) { if (!la.on || b.y > gy + 70) return; var c = nearestOnSeg(b.x, b.z, la.ax, la.az, la.bx, la.bz); if (hyp(b.x - c.x, b.z - c.z) >= K.R + 12) return; if (b.hzCd > 0) return; if (b.shield) { b.shield = false; b.hzCd = 0.5; pop3d(b.x, b.z, b.y, 'BLOCKED!', '#33b6ff'); spark(b.x, b.y + 14, b.z, 12); St.shake = Math.min(8, St.shake + 4); sfx('boost'); return; } var from = b.shotFrom || St.hole.tee; b.x = from.x; b.z = from.z; b.y = St.hole.terrain(from.x, from.z) + K.R; b.vx = b.vy = b.vz = 0; b.air = false; b.settled = true; spark(from.x, b.y + 16, from.z, 12); pop3d(from.x, from.z, b.y, 'ZAPPED!', '#ff4a5a'); St.shake = Math.min(10, St.shake + 6); sfx('tick'); }
  ...
  function syncMeshes() {
    ...
    for (i = 0; i < hole.lasers.length; i++) { var la = hole.lasers[i]; if (la.mesh) { la.mesh.visible = la.on; la.mesh.material.emissiveIntensity = la.on ? .9 : 0; } }
  ...
  function edDown(p, shift) {
    ...
    if (ED.brush === 'wall' || ED.brush === 'laser') {
      ...
      else { var d = ED.draft; edSnapshot(); if (ED.brush === 'wall') d.wall(ED.seg.sx, ED.seg.sz, w.x, w.z, { e: K.wallE }); else d.lasers.push({ ax: ED.seg.sx, az: ED.seg.sz, bx: w.x, bz: w.z, period: 2.2, onFrac: 0.45, phase: 0, on: false }); ED.seg = null; ED.flash = { x: w.x, z: w.z, t: 0.35 }; }
      return;
    }
    if (ED.brush === 'shape') {
      if (!ED.poly) ED.poly = [];
  ...
  function edClosePoly() { if (!ED.poly || ED.poly.length < 2) { ED.poly = null; return; } edSnapshot(); var d = ED.draft, p = ED.poly; for (var i = 0; i < p.length; i++) { var a = p[i], b = p[(i + 1) % p.length]; d.wall(a.x, a.z, b.x, b.z, { e: K.wallE }); } ED.flash = { x: p[0].x, z: p[0].z, t: 0.35 }; ED.poly = null; edPanel(); }
  function edMove(p) {
    if (ED.view3d) {
  ...
  function edFinishDraw() {
    ...
    for (i = 0; i < pts.length - 1; i++) d.wall(pts[i].x, pts[i].z, pts[i + 1].x, pts[i + 1].z, { e: K.wallE });
    if (closed) d.wall(pts[pts.length - 1].x, pts[pts.length - 1].z, pts[0].x, pts[0].z, { e: K.wallE });
  ...
  function drawEditor(c) {
    ...
    ED.draft.lasers.forEach(function (l) { var a = edW2S(l.ax, l.az), b2 = edW2S(l.bx, l.bz); c.strokeStyle = '#ff3a4a'; c.lineWidth = 3; c.beginPath(); c.moveTo(a.x, a.y); c.lineTo(b2.x, b2.y); c.stroke(); });
  ...
  function edPanel() {
    ...
    if (ED.sel && ED.sel.kind === 'wallgroup') {
      ...
      row(gcard, 'height', gi[0] && gi[0].h ? gi[0].h : 46, 20, 130, 4, function (v) { ED.sel.items.forEach(function (g) { g.h = v; }); edLiveRefresh(); });
  ...
  function edSerialize() { var d = ED.draft; return { name: d.name, par: d.par, bounds: d.bounds, tee: d.tee, cup: d.cup, walls: d.walls.filter(function (w) { return !w._bnd; }).map(function (w) { return { ax: w.ax, az: w.az, bx: w.bx, bz: w.bz, e: w.e, h: w.h }; }), bumpers: d.bumpers.map(function (b) { return { x: b.x, z: b.z, r: b.r, kick: b.kick }; }), boosters: d.boosters.map(function (b) { return { x: b.x, z: b.z, ang: Math.atan2(b.dz, b.dx), r: b.r, spd: b.spd }; }), flippers: d.flippers.map(function (f) { return { side: f.side, px: f.px, pz: f.pz, len: f.len, rot: f.rot, speed: f.speed, power: f.power }; }), windmills: d.windmills.map(function (w) { return { x: w.x, z: w.z, r: w.r, n: w.n, speed: w.speed }; }), lasers: d.lasers.map(function (l) { return { ax: l.ax, az: l.az, bx: l.bx, bz: l.bz, period: l.period, onFrac: l.onFrac, phase: l.phase }; }), loops: d.loops.map(function (l) { return { x: l.x, z: l.z, r: l.r, ang: l.ang }; }), warps: d.warps.map(function (w) { return { x: w.x, z: w.z, ex: w.ex, ez: w.ez, r: w.r }; }), portals: d.portals.map(function (w) { return { x: w.x, z: w.z, exits: (w.exits || [{ x: w.ex, z: w.ez }]).map(function (e) { return { x: e.x, z: e.z }; }), r: w.r }; }), firerings: d.firerings.map(function (f) { return { x: f.x, z: f.z, r: f.r, h: f.h, points: f.points }; }), enemies: d.enemies.map(function (e) { return { x: e.x, z: e.z, ex: e.ex, ez: e.ez, r: e.r, speed: e.speed, type: e.type, behavior: e.behavior, effect: e.effect }; }), coins: d.coins.map(function (c) { return { x: c.x, z: c.z, value: c.value }; }), powerups: (d.powerups || []).map(function (p) { return { x: p.x, z: p.z, kind: p.kind }; }), terrain: d.terrainFeatures, noBox: d.noBox, theme: d.theme || 'grass', phys: d.phys || themePhys(d.theme || 'grass'), turf: d.turf, multiball: d.multiball ? { x: d.multiball.x, z: d.multiball.z, r: d.multiball.r } : null }; }
  function edDeserialize(o) { var d = builder(); d.name = o.name || 'LEVEL'; d.par = o.par || 3; d.bounds = o.bounds; d.tee = o.tee; d.cup = o.cup; d.noBox = !!o.noBox; d.theme = o.theme || 'grass'; d.phys = o.phys || themePhys(d.theme); d.turf = o.turf != null ? o.turf : (THEMES[d.theme] || THEMES.grass).turf; rebuildBox(d); (o.loops || []).forEach(function (l) { d.loopde(l.x, l.z, l.r, l.ang); }); (o.warps || []).forEach(function (w) { d.warp(w.x, w.z, w.ex, w.ez, w.r); }); (o.portals || []).forEach(function (w) { d.portal(w.x, w.z, w.exits || (w.ex != null ? [{ x: w.ex, z: w.ez }] : null), w.r); }); (o.firerings || []).forEach(function (f) { d.firering(f.x, f.z, f.r, f.h, f.points); }); (o.enemies || []).forEach(function (e) { d.enemy(e.x, e.z, e.ex, e.ez, e.r, e.speed, e.type, e.behavior, e.effect); }); (o.coins || []).forEach(function (c) { d.coin(c.x, c.z, c.value); }); (o.powerups || []).forEach(function (p) { d.powerup(p.x, p.z, p.kind); }); (o.walls || []).forEach(function (w) { d.wall(w.ax, w.az, w.bx, w.bz, { e: w.e, h: w.h }); }); (o.bumpers || []).forEach(function (b) { d.bumper(b.x, b.z, b.r); if (b.kick != null) last(d.bumpers).kick = b.kick; }); (o.boosters || []).forEach(function (b) { d.booster(b.x, b.z, b.ang, b.r, b.spd); }); (o.flippers || []).forEach(function (f) { d.flip(f.side, f.px, f.pz, f.len, f.rot, f.speed); if (f.power != null) last(d.flippers).power = f.power; }); (o.windmills || []).forEach(function (w) { d.windmill(w.x, w.z, w.r, w.n, w.speed); }); (o.lasers || []).forEach(function (l) { d.lasers.push({ ax: l.ax, az: l.az, bx: l.bx, bz: l.bz, period: l.period, onFrac: l.onFrac, phase: l.phase, on: false }); }); (o.terrain || []).forEach(function (t) { d.terrainFeatures.push(t); }); if (o.multiball) d.mball(o.multiball.x, o.multiball.z, o.multiball.r); return d; }
