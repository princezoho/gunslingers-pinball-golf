...
(function () {
  ...
  function initGL(canvas) {
    ...
    try {
      ...
      var sc = sun.shadow.camera; sc.near = 100; sc.far = 4400; sc.left = -1150; sc.right = 1150; sc.top = 1150; sc.bottom = -1150;   // tight frustum = crisp STABLE shadows (the wide raking-shadow frustum dropped resolution → acne/flicker = the "bouncing lines")
      sun.shadow.bias = -0.0006; if ('normalBias' in sun.shadow) sun.shadow.normalBias = 1.4;   // AAA: low normalBias REATTACHES the cast shadow to each object's contact point (was 3 = detached "floating" shadows); deeper bias (-0.0006) + the tight 4K frustum hold off acne
      R3.scene.add(sun.target); R3.scene.add(sun); R3.sun = sun;
      var rim = new T.DirectionalLight(0xbfe0ff, 0.6); rim.position.set(650, 760, 1050); R3.scene.add(rim); R3.rim = rim; var fill = new T.DirectionalLight(0xffcf8a, 0.28); fill.position.set(700, 380, -200); R3.scene.add(fill); R3.fill = fill; var spot = new T.SpotLight(0xfff0e0, 0, 2000, 0.55, 0.88, 0); spot.position.set(0, 1500, -160); R3.scene.add(spot.target); R3.scene.add(spot); R3.spot = spot; R3.spotBase = 0; R3.sunOff = { x: -1020, y: 760, z: -560 };   // cool back-rim + warm low fill + theatrical SPOTLIGHT. Sun ~33deg = SHORT stable shadows (the ~20deg raking sun looked dramatic but the long grazing shadows acne/flicker on the flat apron = the "bouncing lines")
  ...
  var MOODS = {
    daytime: { name: 'Daylight', exp: 1.26, sun: { c: 0xfff8e8, i: 2.7 }, amb: { c: 0xaec6ee, i: 0.09 }, hemi: { s: 0xb6d4ff, g: 0x4e4530, i: 0.17 }, rim: { c: 0xeaf4ff, i: 0.2 }, fill: { c: 0xffe8c0, i: 0.05 }, fog: 0xcfe2f2, grade: { sat: 1.22, sep: 0, shad: [0.82, 0.9, 1.06], high: [1.1, 1.07, 0.9], lo: 0.16, hi: 0.9, con: 1.34, bright: 0.96, lift: 0.12 }, ca: 0.0014, grain: 0.04, vig: 0.32, godray: 0.45, shadowR: 0.6 },   // harsh HIGH-NOON: blazing white key + lower ambient => darker hard-edged shadows (high ambient is what washed it out), crisp & punchy   // harsh NOON sun: blazing key light + very low ambient so shadows stay DARK & hard-edged (not washed flat)
    sunset: { name: 'Sunset', exp: 0.93, sun: { c: 0xff9d4a, i: 2.3 }, amb: { c: 0x3a3450, i: 0.04 }, hemi: { s: 0xffbe7a, g: 0x252238, i: 0.22 }, rim: { c: 0xffd884, i: 0.5 }, fill: { c: 0x4a64b0, i: 0.22 }, fog: 0x7a5238, grade: { sat: 1.18, sep: 0.08, shad: [0.80, 0.94, 1.15], high: [1.22, 1.05, 0.78], lo: 0.16, hi: 0.8, con: 1.28, bright: 0.94, lift: 0.4 }, ca: 0.0024, grain: 0.06, vig: 0.46, refl: 1.6, godray: 0.55, shadowR: 0.55 },   // teal-orange cinematic split-tone (cool teal shadows vs warm-gold highlights = recognizable graded-film look; standard split-tone, NOT a gradient-map filter)   // LAYERED GOLDEN HOUR via REAL lights (no gradient-map): warmer/brighter amber KEY for raking light + long shadows, HALVED ambient (deep dramatic shadows), deep COOL-BLUE fill on the shadow side (warm/cool separation = the golden-hour signal), hot-gold rim glints, warm-sky→cool-ground hemi, warm amber horizon haze; grass stays GREEN, lit warm   // REAL GRADIENT LIGHTING (no gradient-map filter): blazing warm-orange KEY casting hard shadows + warm-gold rim + COOL-BLUE fill on the shadow side + warm-sky→cool-ground hemi gradient; VERY low ambient = deep dramatic shadows; high contrast, not blown   // PUSHED TOO FAR: blazing golden sun + blooming gold rim, DEEP blue-purple shadows vs blazing orange mids vs hot-gold highlights (gmAmt 0.88), OVERSATURATED, strong orbiting pink/magenta/gold/orange/violet light pools, near-mirror reflective ball   // SPECTACULAR LAYERED SUNSET: golden low sun KEY + pink sky-glow fill + COOL blue-purple rim (warm/cool contrast) + warm-sky→cool-ground hemi gradient + cool-purple→orange→gold gradient map + orbiting warm sunset lights for layered colour & glints; boosted reflections so the ball catches it all
    night: { name: 'Moody Night', exp: 0.64, sun: { c: 0x7e98e0, i: 1.05 }, amb: { c: 0x16223f, i: 0.14 }, hemi: { s: 0x1e2e52, g: 0x04060c, i: 0.34 }, rim: { c: 0xbcd4ff, i: 1.2 }, fill: { c: 0x243056, i: 0.13 }, fog: 0x070a18, grade: { sat: 0.95, sep: 0, shad: [0.28, 0.42, 0.9], high: [0.84, 0.98, 1.18], lo: 0.08, hi: 0.64, con: 1.38, bright: 0.72, lift: 1.5, gmAmt: 0.5, gm: [[0.02, 0.04, 0.12], [0.1, 0.21, 0.48], [0.58, 0.8, 1.0]] }, ca: 0.003, grain: 0.16, vig: 0.68 },   // moody blue night: deepened ambient + brighter cool moon-rim for edge definition; high lift keeps shadows readable (cinematic, not murky); hero ball-spotlight
    noir: { name: 'Film Noir', exp: 1.0, sun: { c: 0xffffff, i: 2.25 }, amb: { c: 0x1a1a1a, i: 0.1 }, hemi: { s: 0x484848, g: 0x0e0e0e, i: 0.26 }, rim: { c: 0xffffff, i: 0.78 }, fill: { c: 0x2c2c2c, i: 0.09 }, fog: 0x1f2226, grade: { sat: 0, sep: 0, shad: [1, 1, 1], high: [1, 1, 1], lo: 0.2, hi: 0.78, con: 1.66, bright: 0.98, lift: 1.0 }, ca: 0.0015, grain: 0.24, vig: 0.7 },   // high-contrast B&W: harder white key + brighter white edge-rim + inkier shadows (global underexpose+contrast deepens it further)
    ...
    vaporwave: { name: 'Vaporwave', exp: 0.7, sun: { c: 0xff1f8f, i: 2.6 }, amb: { c: 0x120426, i: 0.08 }, hemi: { s: 0x3a1080, g: 0x06010f, i: 0.28 }, rim: { c: 0x9a20ff, i: 2.5 }, fill: { c: 0x5a10b8, i: 0.42 }, fog: 0x0a0218, grade: { sat: 1.62, sep: 0, shad: [1, 1, 1], high: [1, 1, 1], lo: 0.14, hi: 0.88, con: 1.32, bright: 0.58, lift: 0.32, gmAmt: 0.8, gm: [[0.015, 0.0, 0.05], [0.42, 0.03, 0.52], [1.0, 0.12, 0.62]], scan: 0.16 }, ca: 0.0086, grain: 0.06, vig: 0.56, lights: 'prism', prismCols: [0xff1f9f, 0x9a2cff, 0xff1f9f, 0xc81fd8, 0x7a1fff], prismI: 6.8 },   // CLASSIC VAPORWAVE: OVERSATURATED purple + neon pink over DEEP BLACKS; black→purple→neon-pink gradient map, hot-pink key + purple rim + orbiting pink/purple neon lights (no cyan), scanlines + CA + bloom
  ...
  function applyMood(name) {
    ...
    if (R3.r.toneMapping) R3.r.toneMappingExposure = m.exp * 0.94;   // gentle underexpose only (was 0.82 — too dark/muddy on wide screens). Clean, brighter look
    ...
    if (R3.post && R3.post.mat) { var u = R3.post.mat.uniforms, g = m.grade; u.uSat.value = g.sat; u.uSep.value = g.sep; u.uShad.value.set(g.shad[0], g.shad[1], g.shad[2]); u.uHigh.value.set(g.high[0], g.high[1], g.high[2]); u.uBloomTint.value.set(1 + (g.high[0] - 1) * 0.7, 1 + (g.high[1] - 1) * 0.7, 1 + (g.high[2] - 1) * 0.7); u.uLo.value = g.lo; u.uHi.value = g.hi; u.uCon.value = g.con * 1.12; u.uBright.value = g.bright; u.uLift.value = g.lift; u.uGrain.value = m.grain; u.uVig.value = m.vig * 0.65; R3.post.ca = m.ca * 0.55;   // AAA: single tone-curve means ×1.12 contrast bites (not crushes); real (clean-centre) vignette + a hint more CA at the corners only
  ...
  var FXLAYERS = {
    cartoon:   { src: 'film-dirt',      op: 0.2 },    // xerox-era cel: vintage film scratches + dirt on the hand-drawn look
    ...
    sunset:    { src: 'bokeh-gold',     op: 0.16 },   // golden-hour bokeh — a subtle dreamy glint
  ...
  function buildWireframe() {
    ...
    if (!R3.wireLineMat) R3.wireLineMat = new T.LineBasicMaterial({ color: 0x40ffe8, fog: false });   // brighter cyan so wireframe/tron edges stay above the bloom threshold after the cinematic underexpose
  ...
  function buildOutline() {
    ...
    if (!R3.outlineMat) R3.outlineMat = new T.ShaderMaterial({ uniforms: { oT: { value: 4.8 }, oCol: { value: new T.Color(0x000000) } }, side: T.BackSide, fog: false, vertexShader: ['uniform float oT;', 'void main(){ vec3 p = position + normalize(normal) * oT; gl_Position = projectionMatrix * modelViewMatrix * vec4(p, 1.0); }'].join('\n'), fragmentShader: ['uniform vec3 oCol;', 'void main(){ gl_FragColor = vec4(oCol, 1.0); }'].join('\n') });
  ...
  function toonSync() {
    ...
    if (on) {
      if (!R3.toonGrad) { R3.toonGrad = toonGrad(4, 0.38, 0.9); R3.toonGradC = toonGrad(2, 0.46, 0.82); }   // deeper shadow band floor (0.42->0.38 cel, 0.5->0.46 cartoon) = richer cel contrast under the cinematic underexpose   // cel = 4 SMOOTH bands (floored+capped so flat colours don't blow); cartoon = 2 hard FLAT bands. Both drop the texture map → flat colour.
  ...
  function initPost() {   // AAA pipeline: scene -> RT, bright-pass -> quarter-res separable gaussian BLOOM, then film composite (CA, grain, vignette, tilt-shift, halation, split-tone grade)
    try {
      ...
      var fsh = [
        ...
        'uniform sampler2D tD; uniform sampler2D tB; uniform float uT; uniform float uCA; uniform float uGrain; uniform float uVig; uniform float uDof; uniform float uFocus; uniform float uBloom; uniform vec3 uBloomTint; uniform vec2 uRes;',
        ...
        'uniform vec2 uSunScreen; uniform float uGodray;',
        ...
        '  col += texture2D(tB, vUv).rgb * uBloom * uBloomTint;',                          // BLOOM: lights actually glow — tinted per mood (warm gold for golden hour, cool for night) so highlights have a cinematic colour glow
        '  if (uGodray > 0.001) { vec2 sd = (uSunScreen - vUv) * (1.0 / 20.0); vec2 suv = vUv; float dk = 1.0; vec3 shaft = vec3(0.0); for (int gi = 0; gi < 20; gi++) { suv += sd; shaft += texture2D(tB, suv).rgb * dk; dk *= 0.93; } col += shaft * (uGodray * 0.05) * uBloomTint; }',   // GOD-RAYS: radial light shafts streaming from the sun — accumulate the bright-pass along the ray toward the sun screen position',
        ...
        '  col = mix(col, col * col * (3.0 - 2.0 * col), 0.0);',                                                   // 5. (DISABLED) — this was a 2nd tone-curve stacked on GPU ACES = the muddy-midtone "flat filter" look; ACES is now the ONLY filmic curve
      ...
      var brightM = new T.ShaderMaterial({ vertexShader: vsh, fragmentShader: brightF, uniforms: { tD: { value: rt.texture }, uThresh: { value: 0.78 } }, depthTest: false, depthWrite: false });   // AAA: 0.88→0.78 so golden-hour speculars (chrome ball, gold ring, wet-grass sheen) actually BLEED a premium glow; the +0.22 soft knee (brightF) keeps bright sand from blowing to white
      ...
      var ssaoM = new T.ShaderMaterial({ vertexShader: vsh, fragmentShader: ssaoF, uniforms: { tDepth: { value: rt.depthTexture }, tNoise: { value: noiseTex }, uProj: { value: new T.Matrix4() }, uProjInv: { value: new T.Matrix4() }, uKernel: { value: ssaoKernel }, uNoiseScale: { value: new T.Vector2(1, 1) }, uRadius: { value: 90 }, uBias: { value: 0.28 }, uStrength: { value: 1.3 } }, depthTest: false, depthWrite: false, extensions: { derivatives: true } });   // AAA: radius 42→90 makes SSAO actually WRAP obstacle bases / curb-contact / ball (was razor-thin = invisible); bias 0.28 restores near-contact occlusion
  ...
  function buildScene(hole) {
    ...
    (function () {
      ...
      var mkMushroom = function (seed) {   // a cluster of toadstools — cream stems, candy caps (the reference's mushroom props)
        ...
        var capM = new T.MeshStandardMaterial({ color: new T.Color().setHSL(0.95 + prnd(seed + 3) * 0.12, 0.62, 0.56), roughness: 0.7 });
        for (var i = 0; i < n; i++) {
          var sh = 18 + prnd(seed + i) * 34, sr = sh * 0.18, cr = sh * 0.52, m = new T.Group();
          var stem = new T.Mesh(new T.CylinderGeometry(sr * 0.8, sr, sh, 24), stemM); stem.position.y = sh / 2; stem.castShadow = true; m.add(stem);
          var cap = new T.Mesh(new T.SphereGeometry(cr, 24, 12, 0, TAU, 0, PI / 2), capM); cap.scale.y = 0.66; cap.position.y = sh; cap.castShadow = true; m.add(cap);
      ...
      var mkTree = function (seed) {   // HIGH-POLY conifer — smooth tapered trunk + many smooth foliage tiers (was 7-seg flat-shaded cones)
        var g = new T.Group(), th2 = 80 + prnd(seed) * 90, tr = th2 * 0.06;
        var trunkM = new T.MeshStandardMaterial({ color: 0x6b4a2b, roughness: 0.9 });
        ...
        var leafM = new T.MeshStandardMaterial({ color: _lc, roughness: 0.85, envMapIntensity: 0.2 });
        var trunk = new T.Mesh(new T.CylinderGeometry(tr * 0.55, tr * 1.2, th2 * 0.44, 16), trunkM); trunk.position.y = th2 * 0.22; trunk.castShadow = true; g.add(trunk);   // smooth flared trunk
        var TIERS = 5;
        for (var i = 0; i < TIERS; i++) { var f = i / (TIERS - 1), tw = th2 * (0.38 - f * 0.30), tht = th2 * 0.32, cone = new T.Mesh(new T.ConeGeometry(tw, tht, 32), leafM); cone.position.y = th2 * (0.34 + f * 0.66) + tht / 2; cone.rotation.y = prnd(seed + i) * 6.283; cone.castShadow = true; cone.receiveShadow = true; g.add(cone); }   // 5 smooth high-seg tiers = round full conifer
        return g;
      };
    ...
    hole.walls.forEach(function (s) { if (s.tunnel) return; var dx = s.bx - s.ax, dz = s.bz - s.az, L = hyp(dx, dz); if (L < 1) return; var g = new T.Group(); var gy = hole.terrain((s.ax + s.bx) / 2, (s.az + s.bz) / 2);   // tunnel rails render as glass below, not plank
      ...
      var capM2 = plankMat(repX, 0.5, prnd2(3) * 0.7, new T.Color(0xe8cda0), .5); var cap = new T.Mesh(new T.CylinderGeometry(13, 13, capW, 32), capM2); cap.rotation.z = PI / 2; cap.position.y = s.h + 2; cap.castShadow = true; g.add(cap); mats.push(capM2);   // ROUNDED rail cap (was a sharp box) — a real curb has a rounded top rail that catches a light highlight
      if (!s.curve) [-1, 1].forEach(function (sgn2) { var post = new T.Mesh(new T.CylinderGeometry(10, 12, s.h + 18, 24), postWoodM); post.position.set(sgn2 * (L / 2 + 3), (s.h + 18) / 2 - 5, 0); post.castShadow = true; g.add(post);   // posts only on straight box walls — an organic curb gets a clean rail instead of a post at every tiny segment
        var pcap = new T.Mesh(new T.SphereGeometry(10.5, 32, 18, 0, TAU, 0, PI / 2), postWoodM); pcap.position.set(sgn2 * (L / 2 + 3), s.h + 13, 0); g.add(pcap); });
    ...
    (hole.tunnels || []).forEach(function (tn) {
      ...
      [[-1, tn.h], [1, tn.h], [-1, 0], [1, 0]].forEach(function (e) { var rail = new T.Mesh(new T.CylinderGeometry(7, 7, tL, 24), steelM); rail.rotation.x = PI / 2; rail.position.set(e[0] * tn.w, e[1], 0); rail.castShadow = true; grp.add(rail); });
      ...
      for (var ri = 0; ri <= ribs; ri++) {
        ...
        if (!tn.roofOnly) [-1, 1].forEach(function (sgn) { var post = new T.Mesh(new T.BoxGeometry(11, tn.h, 11), steelM); post.position.set(sgn * tn.w, tn.h / 2, zz); grp.add(post); var bolt = new T.Mesh(new T.CylinderGeometry(10, 10, 8, 24), boltM); bolt.rotation.x = PI / 2; bolt.position.set(sgn * tn.w, tn.h, zz); grp.add(bolt); });
      }
      R3.group.add(grp);
    });
    ...
    hole.bumpers.forEach(function (bm, bmi) {
      ...
      if (kind === 1) {   // SALOON OIL LANTERN — brass body, real glass chimney, a living flame inside
        ...
        [0.22, 0.5, 0.78].forEach(function (hh) { var hp = new T.Mesh(new T.TorusGeometry(r * (0.78 + Math.sin(hh * PI) * 0.3) + 1.5, 2.6, 20, 80), darkIronM); hp.rotation.x = -PI / 2; hp.position.y = 2 + hh * r * 1.5; g.add(hp); });
    ...
    hole.windmills.forEach(function (wmi) {
      ...
      [-1, 1].forEach(function (sgn) { var post = new T.Mesh(new T.CylinderGeometry(7, 10, wmi.r * 1.2, 20), new T.MeshStandardMaterial({ color: 0x6e4524, roughness: .85 })); post.position.set(wmi.x + sgn * wmi.r * 1.04, gy + wmi.r * 0.6, wmi.z); post.castShadow = true; R3.group.add(post); });
    ...
    (hole.gates || []).forEach(function (ga) {
      ...
      [[ga.ax, ga.az], [ga.bx, ga.bz]].forEach(function (p) { var post = new T.Mesh(new T.CylinderGeometry(15, 19, ga.h + 26, 24), postM); post.position.set(p[0], gy + (ga.h + 26) / 2, p[1]); post.castShadow = true; R3.group.add(post); var cap = new T.Mesh(new T.SphereGeometry(15, 28, 18), goldMat(1.8)); cap.scale.y = .6; cap.position.set(p[0], gy + ga.h + 26, p[1]); R3.group.add(cap); });
    ...
    (hole.pendulums || []).forEach(function (pd) {
      ...
      [-1, 1].forEach(function (s) { var leg = new T.Mesh(new T.CylinderGeometry(11, 15, H - gy, 20), woodM); leg.position.set(pd.x + s * pd.len * 0.5, (gy + H) / 2, pd.z); leg.rotation.z = s * 0.18; leg.castShadow = true; R3.group.add(leg); });
      ...
      var arm = new T.Mesh(new T.CylinderGeometry(4.5, 4.5, pd.len, 16), metalMat(0x8a8e96, 1, 1.1)); arm.position.y = -pd.len / 2; g.add(arm);
    ...
    (hole.turntables || []).forEach(function (tt) {
      ...
      var hub = new T.Mesh(new T.CylinderGeometry(20, 22, 16, 32), metalMat(0x33363d, 1, 1.3)); hub.position.y = 11; g.add(hub);
    ...
    (hole.cannons || []).forEach(function (cn) {
      ...
      var barrel = new T.Mesh(new T.CylinderGeometry(20, 26, 96, 40), ironM); barrel.rotation.x = -PI / 2.6; barrel.position.set(0, 44, 0); barrel.castShadow = true; g.add(barrel);
      var mouth = new T.Mesh(new T.CylinderGeometry(20, 20, 8, 40), new T.MeshBasicMaterial({ color: 0x0a0608 })); mouth.rotation.x = -PI / 2.6; mouth.position.set(0, 44 + Math.sin(PI / 2.6) * 48, Math.cos(PI / 2.6) * 48); g.add(mouth);
      var carriage = new T.Mesh(new T.BoxGeometry(58, 26, 70), woodM); carriage.position.y = 16; carriage.castShadow = true; g.add(carriage);
      [-1, 1].forEach(function (s) { var wheel = new T.Mesh(new T.CylinderGeometry(24, 24, 10, 40), woodM); wheel.rotation.z = PI / 2; wheel.position.set(s * 32, 20, -14); g.add(wheel); });
      var ring = new T.Mesh(new T.TorusGeometry(cn.r, 4, 16, 48), new T.MeshStandardMaterial({ color: 0xc23a2a, emissive: 0x7a1208, emissiveIntensity: .6, roughness: .5 })); ring.rotation.x = -PI / 2; ring.position.y = 3; ring.position.set(0, 3, 0); g.add(ring);
    ...
    (hole.bouncers || []).forEach(function (bc) {
      ...
      var coil = new T.Mesh(new T.TorusGeometry(bc.r * 0.5, 4, 16, 40), goldMat(1.6)); coil.rotation.x = -PI / 2; coil.position.y = 19; g.add(coil);
    ...
    hole.lasers.forEach(function (la) {
      ...
      [[la.ax, la.az], [la.bx, la.bz]].forEach(function (e) { var pg = hole.terrain(e[0], e[1]), post = new T.Mesh(new T.CylinderGeometry(13, 16, 64, 20), new T.MeshStandardMaterial({ color: 0x3a1418, emissive: 0x6a0a12, emissiveIntensity: .5, roughness: .6 })); post.position.set(e[0], pg + 32, e[1]); post.castShadow = true; R3.group.add(post); });
    });
    // multiball pad
    ...
    (hole.warps || []).forEach(function (wp) {
      ...
      if (wp.tube) {   // a polished-STEEL pipe that arcs from the entrance up and over to the exit (metal, not wood)
        ...
        for (var tb = 1; tb < 5; tb++) { var tp = crv.getPoint(tb / 5), band = new T.Mesh(new T.TorusGeometry(R * 1.06, R * 0.13, 16, 64), bandM); var tng = crv.getTangent(tb / 5); band.position.copy(tp); band.lookAt(tp.clone().add(tng)); R3.group.add(band); }
        var rimM = metalMat(0x6a6f78, 1, 1.2);
        [[wp.x, gy, wp.z], [wp.ex, gy2, wp.ez]].forEach(function (e, ei) { var mouth = new T.Mesh(new T.CylinderGeometry(R * 1.08, R * 1.08, 26, 32, 1, true), rimM); mouth.position.set(e[0], e[1] + R * 0.5, e[2]); R3.group.add(mouth); var hole2 = new T.Mesh(new T.CircleGeometry(R * 0.92, 32), new T.MeshBasicMaterial({ color: 0x0a0608 })); hole2.rotation.x = -PI / 2; hole2.position.set(e[0], e[1] + 2.5, e[2]); R3.group.add(hole2); if (ei === 0) wp.mesh = mouth; });
        return;
      }
      ...
      var ring1 = new T.Mesh(new T.TorusGeometry(wp.r, 6, 16, 48), new T.MeshStandardMaterial({ color: 0x2aa8ff, emissive: 0x1466cc, emissiveIntensity: .85, roughness: .35 })); ring1.rotation.x = -PI / 2; ring1.position.set(wp.x, gy + 2, wp.z); R3.group.add(ring1); wp.mesh = ring1;
      var ring2 = new T.Mesh(new T.TorusGeometry(wp.r * .9, 5, 16, 48), new T.MeshStandardMaterial({ color: 0x2aff9a, emissive: 0x12a866, emissiveIntensity: .75, roughness: .35 })); ring2.rotation.x = -PI / 2; ring2.position.set(wp.ex, gy2 + 2, wp.ez); R3.group.add(ring2);
    ...
    var pole = new T.Mesh(new T.CylinderGeometry(2.8, 3.8, 232, 24), new T.MeshStandardMaterial({ color: 0xf0ebe0, roughness: .5 })); pole.position.set(cu.x, cy - cupD + 116, cu.z); pole.castShadow = true; R3.group.add(pole);   // pole foot rests at the cup bottom
