...
(function () {
  ...
  function initGL(canvas) {
    ...
    try {
      ...
      R3.prism = []; var prismCol = [0xff1f8f, 0x12d8ff, 0xffb02e, 0x9b5cff, 0x39ff9c];   // REAL coloured lights (magenta / cyan / amber / violet / lime) that orbit the table and ADDITIVELY illuminate the actual geometry — prismatic looks come from light, not a filter
      for (var _pi = 0; _pi < prismCol.length; _pi++) { var _pl = new T.PointLight(prismCol[_pi], 0, 1900, 1.1); _pl.position.set(0, 300, 0); R3.scene.add(_pl); R3.prism.push(_pl); } R3.prismOn = false; R3.prismI = 0;
